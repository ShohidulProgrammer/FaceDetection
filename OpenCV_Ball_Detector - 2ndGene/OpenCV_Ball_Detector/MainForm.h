...
namespace OpenCV_Ball_Detector {
	...
	const int FRAME_WIDTH = 320;	// Video frame
	const int FRAME_HEIGHT = 240;	// Video fram size
	const int MIN_OBJECT_AREA = 10 * 10;	// Min object need to tracking area spec
			...
			if (hier.size() > 0)	// hier: number of object detected after contours processing.
			{
				...
				if (obj_qty < Max_obj)	//Only process when number of obj lower than define max obj.
				{
					for (int idx = 0; idx >= 0; idx = hier[idx][0]) {
						...
						{
							xm = moms.m10 / area; // if found obj we can get x,y ROI position.
							ym = moms.m01 / area;
							objFound = true;
							arearef = area;
						}
				...
				{
					putText(srcImg, "Object Found", cv::Point(0, 50), 2, 1, Scalar(0, 0, 255), 2); // Put text on the video
					circle(srcImg, cv::Point(xm, ym), 20, Scalar(0, 255, 0), 2, 8, 0); // Draw cirle on object
					putText(srcImg, n2str.int2str(xm) + n2str.int2str(ym), cv::Point(xm, ym), 2, 1, Scalar(0, 255, 0), 2); // write x,y ROI on found obj
				}
